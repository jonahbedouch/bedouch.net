import { BlogFrontmatter } from '@/helpers/frontmatter.helper';
...
async function ContentCard(props: Props) {
  ...
  return <article className=''>
    <Link href={`/blog/${props.frontmatter.slug}`} className='rounded-lg bg-secondary-100 hover:bg-secondary-0 dark:base:bg-secondary-900 dark:bg-secondary-950 dark:base:hover:bg-secondary-1000 dark:hover:bg-secondary-1000 border border-secondary-700 dark:border-secondary-500 my-2 lg:pl-2 p-1 group flex flex-row relative h-40 w-full hover:shadow-low dark:hover:shadow-d-low transition-all duration-200'>
      <div className="flex-col flex-grow w-1">
        ...
        <span className='text-secondary-700 dark:text-secondary-400'>{pubdate.toLocaleDateString('en-US', { month: "long", day: "numeric", year: "numeric" })}</span>

...
date: "2023-01-19"
featureImage: "/src/content/projects/svelte-site/images/website-thumb.webp"
excerpt: ""
tags:
  - "Web"
  - "Svelte"
...
posts and project writeups are written in `.mdx` files, which are component-enhanced markdown files similar to traditional 
`.mdx` files in React. This markdown is fed through some remark and rehype plugins for formatting, then made available 
to Svelte through a SvelteKit server-only api endpoint. From there, when a user queries the `blogs/[slug]` path or the 
`projects/[slug]` path the page sends a request to the endpoint for the relevant markdown content, then loads it alongside 
the necessary styling for the page. This is the bulk of the core functionality of the site, but there are also some fun 
additional features which I included while designing it.
...
that I never got around to designing. For now, though, I just wanted to get something out there. I hope you'll stick 
along for the journey as the website improves alongside the amount and quality of content. If you have any suggestions 
for improvements that I could make, feel free to email me at [jonah@bedouch.net](mailto:jonah@bedouch.net).
...
export async function generateMetadata({
  ...
  try {
    const path = await getCachedFileLocation("blog", slug);
    if (path === null) {
      throw Error();
    }
  ...
  const { toc, content, frontmatter } = await compilePageMdx<BlogFrontmatter>(
    contentStr,
    "/blog-assets/",
  );
  ...
  let openGraph: Metadata["openGraph"] = {
    ...
    url: `https://bedouch.net/blog/${slug}`,
  ...
  if (frontmatter.thumbnail) {
    openGraph.images = [
      { url: `/blog-assets/${frontmatter.slug}/${frontmatter.thumbnail}` },
    ];
    twitter.images = [
      `/blog-assets/${frontmatter.slug}/${frontmatter.thumbnail}`,
    ];
  }

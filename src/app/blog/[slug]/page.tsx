import { BlogFrontmatter, cachedReadFile, getCachedFileLocation, getCachedSlugs } from "@/helpers/frontmatter.helper";
...
export async function generateStaticParams() {
  const posts = await getCachedSlugs('blog');
...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  ...
  try {
    const path = await getCachedFileLocation('blog', params.slug);
  ...
  if (frontmatter.published === false) { return notFound(); }
  let openGraph: Metadata["openGraph"] = {
    title: `${frontmatter.title} | Jonah Bedouch`,
    description: frontmatter.excerpt,
    url: `https://bedouch.net/blog/${params.slug}`,
  ...
  if (frontmatter.thumbnail) {
    openGraph.images = [{ url: `/blog-assets/${frontmatter.slug}/${frontmatter.thumbnail}` }]
    twitter.images = [`/blog-assets/${frontmatter.slug}/${frontmatter.thumbnail}`]
  }
    ...
    <main className="w-full">
      <Suspense fallback={<></>}>
        <hgroup className="lg:py-sm px-sm py-md mt-3xs-xl md:col-span-9 col-span-12 bg-secondary-0 dark:bg-secondary-1000 overflow-hidden rounded-lg shadow-medium dark:shadow-d-medium ring-1 ring-secondary-1000 dark:ring-secondary-900 ring-opacity-5" aria-labelledby={`section-${frontmatter.title}`}>
          ...
          <span className="sr-only">Published on: {published.toLocaleDateString('en-us', { timeZone: "UTC", month: "long", day: "numeric", year: "numeric" })}</span>
          <p className="text-center" aria-hidden={true}>{published.toLocaleDateString('en-us', { timeZone: "UTC", month: "short", day: "numeric", year: "numeric" })}</p>
        </hgroup>
